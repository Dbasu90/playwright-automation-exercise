...
import cardDetails from '../../test-data/cardDetails.json' with { type: 'json' };
...
test('Verify order summary on checkout and place order', async ({ cartPage, userDetails, productsPage }) => {
    ...
    await test.step('Verify address and order summary is displayed', async () => {
        ...
        await expect(checkout.billingAddress).toBeVisible();
...
test('Submit Contact Us Form', async ({ cartPage }) => {
    const msg = await cartPage.navBar.submitContactUs();
    await expect(msg).toHaveText('Success! Your details have been submitted successfully.');
});

test('Subscription', async ({ page }) => {
    await page.locator('#susbscribe_email').fill(process.env.EMAIL!);
    await page.locator('#subscribe').click();
    await expect(page.locator('#success-subscribe div')).toHaveText('You have been successfully subscribed!');
});
