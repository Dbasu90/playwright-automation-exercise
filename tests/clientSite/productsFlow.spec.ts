...
test.describe('Add Products to Cart', () => {
    ...
    test('Verify product details page and add product to cart', async ({ productsPage }) => {
        ...
        await test.step('Verify Cart contains both items', async () => {
            ...
            await expect(cartPage.cartDescription).toHaveText([productName, anotherProduct]);
        });
    });
    ...
    test('Verify cart displays added products', async ({ cartPage, page, productsPage }) => {
        ...
        await test.step('Refresh and verify the cart is updated', async () => {
            await page.reload();
            await expect(cartPage.cartDescription).toHaveText([productName]);
            await cartPage.clickOnRemove(productName);
        });
    });

    test('Verfiy user can submit a review for a product', async ({}) => {
